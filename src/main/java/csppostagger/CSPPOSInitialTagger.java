 ...
 * 
 */
public class CSPPOSInitialTagger
{
	private static final Pattern QUOTATION = Pattern.compile("(“)|(”)|(\")");

	public static List<CSPPOSWordTag> InitTagger4Sentence(
		HashMap<String, String> DICT, String sentence)
	{
		List<CSPPOSWordTag> wordtags = new ArrayList<CSPPOSWordTag>();

		for (String word : sentence.split("\\s+")) {
			if (QUOTATION.matcher(word).find()) {
				// wordtags.add(new WordTag("''", DICT.get("''")));
				if (DICT.containsKey("''")) { 
					wordtags.add(new CSPPOSWordTag("''", DICT.get("''")));
				}
			...
			wordtags.add(new CSPPOSWordTag(word, tag));
		}
		return wordtags;
	}
	...
	public static List<CSPPOSWordTag> EnInitTagger4Sentence(
		HashMap<String, String> DICT, String sentence)
	{
		List<CSPPOSWordTag> wordtags = new ArrayList<CSPPOSWordTag>();
		...
		throws IOException
	{
		...
		for (String line; (line = buffer.readLine()) != null;) {
			...
			for (CSPPOSWordTag st : EnInitTagger4Sentence(DICT, line))
	...
	public static List<CSPPOSWordTag> VnInitTagger4Sentence(
		HashMap<String, String> DICT, String sentence)
	{
		List<CSPPOSWordTag> wordtags = new ArrayList<CSPPOSWordTag>();
		...
		throws IOException
	{
		...
		for (String line; (line = buffer.readLine()) != null;) {
			...
			for (CSPPOSWordTag st : VnInitTagger4Sentence(DICT, line))
				bw.write(st.word + "/" + st.tag + " ");
			bw.write("\n");
		}
